...
if ( ! function_exists( 'organic_shortcodes_enqueue_scripts' ) ) {
	...
	function organic_shortcodes_enqueue_scripts() {
		...
		// Enqueue Styles.
		wp_enqueue_style( 'organic-shortcodes-styles', plugin_dir_url( dirname( __FILE__ ) ) . 'organic-shortcodes/css/organic-shortcodes.css', array(), '1.0' );
		wp_enqueue_style( 'organic-shortcodes-font-awesome', plugin_dir_url( dirname( __FILE__ ) ) . 'organic-shortcodes/css/font-awesome.css', array( 'organic-shortcodes-styles' ), '1.0' );

		// Resgister Scripts.
		wp_register_script( 'organic-shortcodes-modal', plugin_dir_url( dirname( __FILE__ ) ) . 'organic-shortcodes/js/jquery.modal.min.js', array( 'jquery' ), '1.0', true );

		// Enqueue Scripts.
		wp_enqueue_script( 'organic-shortcodes-script', plugin_dir_url( dirname( __FILE__ ) ) . 'organic-shortcodes/js/jquery.shortcodes.js', array( 'jquery', 'organic-shortcodes-modal', 'jquery-ui-accordion', 'jquery-ui-dialog' ), '1.0', true );

		// Enqueue jQuery UI Tabs code only if not in the Customizer.
		// http://core.trac.wordpress.org/ticket/23225.
		global $wp_customize;
		if ( ! isset( $wp_customize ) ) {
			wp_enqueue_script( 'organic-shortcodes-tabs', plugin_dir_url( dirname( __FILE__ ) ) . 'organic-shortcodes/js/tabs.js', array( 'jquery', 'jquery-ui-tabs' ), '1.0', true );
		}
